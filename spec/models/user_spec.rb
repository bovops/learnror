  ...
  describe '.find_for_facebook_oauth' do
    ...
    context 'user already has identity' do
      it 'returns the user' do
        ...
        expect( User.find_for_facebook_oauth(auth) ).to eq user
      end
    end
    ...
    context 'user has not identity' do
      context 'user already exists' do
        ...
        it 'creates identity for user' do
          expect { User.find_for_facebook_oauth(auth) }.to change(user.identities, :count).by(1)
        end

        it 'creates identity for user with provider and uid' do
          identity = User.find_for_facebook_oauth(auth).identities.first
          expect(identity.provider).to eq auth.provider
          expect(identity.uid).to eq auth.uid
        end

        it 'returns the user' do
          expect( User.find_for_facebook_oauth(auth) ).to be_a(User)
        end
      end
